    ...
    <div className="bg-linear-to-br from-[#0b57cd] to-[#0947ab] p-4 relative">
      ...
      {ens.photoUrl ? (
        <img
          src={ens.photoUrl}
          alt={`${ens.prenom} ${ens.nom}`}
          className="w-12 h-12 rounded-xl object-cover mb-2 shadow-sm border-2 border-white/30"
        />
      ) : (
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          >
            <p className={`text-[13px] font-black ${s.color} leading-none`}>
              {s.value}
            </p>
      ...
      <div className="space-y-2.5">
        ...
        {[
          ...
          {
            icon: Briefcase,
            label: CONTRAT_CFG[ens.typeContrat]?.label ?? ens.typeContrat,
          },
        ].map(({ icon: Icon, label }) => (
          <div key={label} className="flex items-start gap-2.5">
            <Icon className="w-3.5 h-3.5 text-gray-300 mt-0.5 shrink-0" />
            <span className="text-[12px] text-gray-600 break-all">{label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-2 pt-1">
        ...
        <button
          ...
          disabled={isUpdating}
          className="flex items-center justify-center gap-2 py-2 bg-gray-50 text-gray-700 rounded-lg text-[12px] font-semibold hover:bg-gray-100 transition-colors border border-gray-200 disabled:opacity-50"
        >
          {ens.actif ? (
            <>
            ...
            <>
              <ToggleRight className="w-3.5 h-3.5 text-[#0b57cd]" /> Activer
            </>
          )}
        </button>
        <button
