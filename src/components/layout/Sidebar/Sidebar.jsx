...
const baseMenuItems = [
  ...
  // ── Paramètres avec sous-menu réservé au directeur ──
  {
    ...
    children: [
      ...
      { title: "Utilisateurs", path: "/parametres/utilisateurs" },
      { title: "Rôles & Permissions", path: "/rolepermission", icon: UserKey },
    ],
  },
];
...
const Sidebar = ({ collapsed, setCollapsed }) => {
  ...
  // Ouvre le sous-menu si forcé par l'utilisateur OU si un enfant est actif
  ...
  const SidebarInner = () => (
    <div className="flex flex-col h-full">
                    ...
                    <div key={item.title}>
                      ...
                      >
                        ...
                        {!collapsed && (
                          <>
                            <span
                              className={`flex-1 text-left text-[13.5px] ${active ? "font-semibold text-white" : "font-normal"}`}
                            >
                              {item.title}
                            </span>
                              ...
                              {item.children.map((child) => {
                                ...
                                return (
                                  ...
                                  >
                                    {ChildIcon ? (
                                      <ChildIcon
                                        className={`w-3.5 h-3.5 shrink-0 ${childActive ? "text-white" : "text-white/40"}`}
                                      />
                                    ) : (
                                      <div
                                        className={`w-1.5 h-1.5 rounded-full shrink-0 ${childActive ? "bg-white" : "bg-white/35"}`}
                                      />
                                    )}
                                    {child.title}
                                  </button>
      ...
      <div className={`px-3 pb-5 ${collapsed ? "flex justify-center" : ""}`}>
        <div
          className={`flex items-center gap-3 ${collapsed ? "" : "bg-white/10 rounded-xl px-3.5 py-3 border border-white/20"}`}
        >
          <div className="relative shrink-0">
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className={`fixed lg:relative z-40 h-screen flex flex-col shadow-2xl shadow-blue-900/40 shrink-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <button
